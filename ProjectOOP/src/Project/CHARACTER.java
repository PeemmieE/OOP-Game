...
abstract class CharMain{
    abstract void run();
    abstract void jump();
    abstract void slide();
}

public class CHARACTER extends CharMain {
    ...
    CHARACTER(){
        for (int i = 0; i < 12; i++) {
            String name;
            if (i < 10) {
                name = "0_Piffle_Run_00"+i+".png";
            }
            else{
                name = "0_Piffle_Run_0"+i+".png";
            }
            runpic[i] = new ImageIcon(this.getClass().getResource(name));
        }
        for (int i = 0; i < 6; i++) {
            String name = "0_Piffle_Slide_00"+i+".png";
            slidepic[i] = new ImageIcon(this.getClass().getResource(name));
        }
        for (int i = 0; i < 6; i++) {
            String name = "0_Piffle_Run_00"+i+".png";
